...
/*   Updated: 2024/04/30 13:19:47 by chang-pa         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	mbe_execute_node(t_astree *node)
{
	...
	if (node->type == TK_COMMAND && mbe_simple_cmd(node) != 0)
		...
		return (-1);
	else if (node->type == TK_REDIRECT_APPEND && mbe_redirect_append(node) != 0)
		return (-1);
	else if (node->type == TK_REDIRECT_HEREDOC && mbe_redirect_heredoc(node) != 0)
		return (-1);
	return (0);
}
